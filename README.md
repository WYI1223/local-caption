...
Windows 可在 ··· 菜单独立勾选“保存原始音频（下次开始生效）”，每次启动应用默认关闭，需要时手动开启，适用于麦克风和系统声音。它保存增强及单声道混音之前的设备音频，以设备采样率、原声道数转换为 PCM16 WAV，和字幕同名放在 `transcripts`。录制中是 `.partial.wav`，停止并完成写盘后改成 `.wav`；字幕末尾也会显示音频路径。该选项独立于字幕自动保存，不会补回开启前或软件停止采集后的音频；不是原始浮点采样的无损副本。异常中断可能留下 `.partial.wav`，应保留并检查，不能直接认定完整。48 kHz 双声道约 11.5 MB/分钟。macOS 音频保存暂未接入。
...
```bat
setup.cmd --experimental-denoise
```

安装后重启应用，在 `···` 菜单选择原版、**实验性 25% GTCRN＋75% 原音**或**实验性 50% GTCRN＋50% 原音**，下次开始生效。新安装默认原版；已选模式会记住。依赖和模型未安装时实验选项禁用，普通识别仍可使用。当前支持 Windows 麦克风／系统声音的 16 或 48 kHz 输入，macOS 暂不启用此选项，欢迎单独提交移植 PR。

这两个选项只做降噪回混，**不加动态增益**。降噪听起来更干净，不保证识别更准确：实测有术语改善，也有弱语音、否定或公式受损。建议用同一段声音分别测试，以具体句子对照，不以总词数判断准确率。保存原始音频仍需单独勾选，保存的是增强前设备音频。

欢迎在 GitHub 新建 Issue，选择 **Experimental denoising feedback** 模板，填写系统／CPU、输入设备、采样率、模式、时长及具体改善或退步的句子。注明是否出现积压、停止后是否清空。反馈由用户手动提交，程序没有自动上传或遥测；只分享有权公开且已脱敏的片段，不需要上传整堂课录音。更换模式时先停止，再从同一位置播放。
