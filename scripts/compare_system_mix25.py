"""Same captured PCM sources and native ASR as mix50; 25% GTCRN, no gain."""
import datetime
import hashlib
import html
import json
...
def main():
    ...
    x=(raw*.75+gt*.25).astype(np.float32)
    assert len(x)==len(raw)==len(gt) and np.isfinite(x).all()
    out=source/('mix25-'+datetime.datetime.now().strftime('%Y%m%d-%H%M%S'));out.mkdir()
    np.save(out/'mix25-asr-input.npy',x);save_wav(out/'mix25.wav',x)
    report=dict(source_sha256=hashes,baseline_report=str(previous/'results.json'),
        mixing='0.75 * raw + 0.25 * GTCRN; no gain, no RMS matching',audio_seconds=len(x)/16000,
    ...
    try:
        with (out/'mix25-events.jsonl').open('w',encoding='utf-8') as log:
            def consume(results):
            ...
            for i in range(0,len(x),1600):
                ...
                if i%960000==0:print('mix25',round(i/16000),flush=True)
    ...
    (out/'mix25.txt').write_text(history.text,encoding='utf-8')
    (out/'results.json').write_text(json.dumps(report,ensure_ascii=False,indent=2),encoding='utf-8')
    options=[('原版',f'../{previous.name}/raw_pcm',baseline['asr']['raw_pcm']),
             ('25% GTCRN（75% 原音）','mix25',report['asr']),
             ('50% GTCRN（50% 原音）',f'../{previous.name}/mix50',baseline['asr']['mix50'])]
    parts=['''<!doctype html><meta charset="utf-8"><title>25% GTCRN 对照</title><style>body{font:16px system-ui;max-width:1150px;margin:30px auto;padding:15px;background:#f3f5f9;color:#17253a}section{padding:18px;background:white;margin:15px 0}audio{width:100%}p{line-height:1.8}button{padding:10px;margin:5px}td,th{padding:12px;border:1px solid #ddd}table{border-collapse:collapse;width:100%}</style><h1>同一十分钟录音：原版／25%／50% GTCRN</h1><p>比例指 GTCRN 在波形混合中的占比，均不加动态增益。复用相同 PCM 源文件与相同 Nemotron Q8 CPU 流式配置，只新运行 25% 版本；原版和 50% 结果来自上一轮。没有人工真值，词数不是准确率；本轮不是实时采集或队列测试。</p><section><audio id="player" controls src="mix25.wav"></audio><p id="label">25% GTCRN</p>''']
    ...
    for parent,link in [(source/'compare.html',out.name+'/compare.html'),(previous/'compare.html','../'+out.name+'/compare.html')]:
        with parent.open('a',encoding='utf-8') as f:f.write(f'<section><h2>25% 补测</h2><a href="{link}">查看原版／25%／50% 对照</a></section>')
